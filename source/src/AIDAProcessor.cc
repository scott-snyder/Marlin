...
AIDAProcessor::AIDAProcessor()
  ...
  registerProcessorParameter("FileType",
                             ...
                             _fileType, std::string("root"));

  registerProcessorParameter("FileName", " filename without extension",
                             _fileName, std::string("aida_file"));
...
void AIDAProcessor::init() {
  ...
  std::string sexport(
      "root"); // root or Lab  - only meaningful for _fileType==root
...
void AIDAProcessor::check(LCEvent *) {
  ...
  if (isFirstEvent()) {
    ...
    hEvtTime = _histoFactory->createCloud1D("hEvtProcessingTime",
...
AIDAProcessor::GetIAnalysisFactory(const Processor *proc) {
  tree(proc);
  return _me->_analysisFactory;
}

AIDA::IHistogramFactory *
AIDAProcessor::histogramFactory(const Processor *proc) {
  tree(proc);
...
AIDAProcessor::dataPointSetFactory(const Processor *proc) {
  tree(proc);
...
AIDA::ITree *AIDAProcessor::tree(const Processor *proc) {
  ...
  if (!_me) {
    throw Exception(" AIDA is not properly initialized - you need to have the "
                    "AIDAProcessor as"
